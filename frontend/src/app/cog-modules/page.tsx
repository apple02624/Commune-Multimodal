...
import { ModuleItemProps } from "@/components/molecules/cog-modules";
...
const CogModulePage = () => {
  ...
  const [isShowPolkadotWalletModalOpen, setIsShowPolkadotWalletModalOpen] =
    useState(false);
  ...
  async function fetchModules(val: string) {
    const response = await fetch(`/api/replicate?cursor=${val}`, {
      method: "GET",
    });
  ...
  return (
    <>
      <main className="mt-[30px] flex flex-col items-center justify-center my-auto mx-auto xl:w-[1400px] px-[20px] ">
        <div
          className=" mb-[20px] bg-blue-700 rounded-lg shadow-lg hover:shadow-2xl text-center hover:bg-blue-600 duration-200 
          ...
          onClick={handlePolkadotWalletModal}
        >
          <Image
            style={{ width: "auto", height: "2.7rem", marginRight: "0.25rem" }}
        ...
        {displayedModules && displayedModules.length > 0 ? (
          <div className="mt-[40px] grid grid-cols-3 gap-[20px] w-[100%]">
            {/* {displayedModules.map((item: ModuleItemProps, idx) => (
      ...
      {isShowPolkadotWalletModalOpen && (
        <Modal
          open={isShowPolkadotWalletModalOpen}
          onCancel={handleShowPolkadotWalletModalCancel}
          footer={null}
          width={500}
        >
